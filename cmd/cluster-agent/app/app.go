...
func start(cmd *cobra.Command, args []string) error {
	...
	retrierCtx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()
	apiCl, err := apiserver.WaitForAPIClient(retrierCtx) // make sure we can connect to the apiserver
	if err != nil {
